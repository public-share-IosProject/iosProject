    ...
    vm = new Vue({
        ...
        methods: {
            loadCurItem: function() {
                var data = wb.getData();
                if (data.release_curItem !== undefined &&
                    data.release_curItem !== null &&
                    data.release_curItem !== '') {
                    this.curItem = data.release_curItem;
            ...
            loadDownData: function(me, status) {
                ...
                if (self.finish[self.itemIndex] !== true) {
                    var params = {
                        ...
                        url: 'http://www.renrenshangjin.com/index.php?m=Home&c=UserTask&a=getUserRelease',
                        status: status
                    };
                ...
                switch (self.itemIndex) {
                    case 0:
                        items.forEach(function(item) {
                            ...
                            result += self.htmlConcat(item, time, false);
                        });
                        break;
                    case 1:
                        items.forEach(function(item) {
                    ...
                    case 2:
                        items.forEach(function(item) {
                            ...
                            var isReddot = (item.apply == 0) ? true : false;

                            result += self.htmlConcatReddot(item, time, isReddot);
                        });
                        break;
                    case 3:
                        items.forEach(function(item) {
                            var date = new Date(item.updated_at * 1000);
                            var time = date.getFullYear() + '.' + (date.getMonth() + 1) + '.' + (date.getDate()) + ' ' + date.getHours() + ':' + date.getMinutes();

                            var isReddot = (item.is_evaluate == 0) ? true : false;
             ...
             * @return
             */
            htmlConcatReddot: function(item, time, flag) {
                var isReddot = "";

                if (flag) {
                    isReddot = '<i class="ui-reddot ui-reddot-static" style="margin-top: 5px;"></i>';
                } else {
                    isReddot = '<i class="white-dot white-dot-static" style="margin-top: 5px;"></i>';
                }
            ...
            keepCurrentData: function(task_id) {
                ...
                data.myReleaseData_task_id = task_id;
                data.release_curItem = self.itemIndex;
                ...
                wb.setData(data, function() {
                    ...
                    switch (self.itemIndex) {
                        case 0:
                            addr = 'demandReleasing.html';
                            break;
                        case 1:
                            addr = 'demandReleased.html';
                            break;
                        case 2:
                            addr = 'demandReleaseUnsolved.html';
                            break;
                        case 3:
                            addr = 'demandReleaseSolved.html';
                            break;
                    }
             ...
             * @param  {number} secs  the item's time
             * @return
             */
            clock: function(item, secs) {
                var self = this;

                setInterval(function() {
                    if (--secs > 0) {
                        Vue.set(item, 'restTime', self.format(secs));
                    } else {
                        self.items[self.itemIndex].$remove(item);
                    }
                }, 1000);
            },
